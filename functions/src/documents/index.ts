 ...
 */
export const checkExpiringDocuments = onSchedule(
  { region: REGION, schedule: SCHEDULE, timeZone: TZ, timeoutSeconds: 540 },
  async () => {
    ...
    const byAgency = new Map<
      string,
      Array<{ docId: string; clientId: string; type: string; validUntil: Date; fileName: string }>
    >();
    ...
    for (const [agencyId, docs] of byAgency) {
      ...
      for (const d of docs) {
        const daysLeft = Math.ceil(
          (d.validUntil.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
        );
