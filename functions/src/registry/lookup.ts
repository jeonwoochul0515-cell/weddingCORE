...
export type BrokerHit = {
  entrpsNm: string;
  rprsvNm: string;
  region: string; // `${ctpvNm} ${sggNm}` 또는 ctpvNm
...
function normalize(raw: BrokerRaw): BrokerHit {
  ...
  return {
    ...
    location:
      typeof raw.lat === 'number' && typeof raw.lot === 'number'
 ...
 * - 운영 중(operYn=Y)만 반환
 */
export const searchBrokerRegistry = onCall(
  { region: REGION, secrets: [DATA_GO_KR_KEY], timeoutSeconds: 15, memory: '256MiB' },
  async (request) => {
    if (!request.auth?.uid) {
      throw new HttpsError('unauthenticated', '로그인이 필요합니다.');
    }
    const keyword = String((request.data as { keyword?: string })?.keyword ?? '').trim();
    if (keyword.length < 2) {
    ...
    let json: {
      response?: {
        ...
        body?: {
          totalCount?: number;
          items?: { item?: BrokerRaw[] | BrokerRaw };
        };
      };
    };
    try {
    ...
    const arr: BrokerRaw[] = Array.isArray(rawItems) ? rawItems : rawItems ? [rawItems] : [];
    const hits = arr
      .map(normalize)
 ...
 * - 야간 sync 함수에서도 재사용
 */
export async function fetchBrokerByName(entrpsNm: string): Promise<BrokerHit | null> {
  ...
  const json = (await res.json()) as {
    response?: { body?: { items?: { item?: BrokerRaw[] | BrokerRaw } } };
  };
  const rawItems = json.response?.body?.items?.item;
  const arr: BrokerRaw[] = Array.isArray(rawItems) ? rawItems : rawItems ? [rawItems] : [];
