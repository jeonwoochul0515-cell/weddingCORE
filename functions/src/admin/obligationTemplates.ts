...
export type ObligationTemplateSeed = {
  code: string;
  stage: 1 | 2 | 3 | 4 | 5 | 6;
  ...
  legalBasis: string;
  penaltyRange: string;
  disputeRisk: string; // 분쟁 시나리오 (변호사 관점)
  dueDateRule: {
    type: 'relative' | 'absolute';
    anchor: 'client_created' | 'contract_signed' | 'meeting_scheduled' | 'marriage_registered' | 'entry_date';
    offsetDays: number;
  };
...
export const OBLIGATION_TEMPLATES: ObligationTemplateSeed[] = [
  // ===== Stage 1: 계약 전 (5) =====
  {
    ...
    description:
      '결혼중개업법 제3조에 따라 국내/국제가 구분 등록. 국제결혼중개업은 자본금 1억원 이상, ' +
      '보증보험 가입, 교육이수 등 요건 충족 필수. 등록 관할은 특별자치시·시·군·구청.',
    legalBasis: '결혼중개업법 제3조, 제4조, 시행령 제3조 (국제결혼중개업 등록요건)',
    penaltyRange:
      '미등록 영업: 3년 이하 징역 또는 3천만원 이하 벌금 (§26). ' +
      '등록요건 미달: 영업정지 → 등록취소 (§18).',
    disputeRisk:
      '무등록 상태에서 체결된 중개계약은 민법 §103(반사회질서) 또는 §746(불법원인급여) ' +
  ...
  },
  {
    ...
    description:
      '국제결혼중개업자는 1억원 이상 보증보험 의무 가입. 보험기간 단절 시 신규 계약 체결 금지. ' +
      '보험사: 서울보증보험 등 금융위 인가 보증보험사.',
    legalBasis: '결혼중개업법 제24조의2, 시행령 제3조의2',
    penaltyRange:
      '미가입·중단: 과태료 500만원 이하 + 영업정지 3개월 → 등록취소.',
    disputeRisk:
      '보험 미가입 상태에서 고객 피해 발생 시 업체 파산 시 실질 배상 불능. ' +
  ...
  // ===== Stage 2: 계약 체결 (7) =====
  {
    ...
    title: '서면계약 체결 (§10)',
    description:
      '계약 체결 시 반드시 서면. 필수 기재사항 11개: 중개범위, 회원자격, 수수료 금액·지급시기, ' +
      '환불조건(단계별), 계약기간·갱신, 손해배상 책임, 중개인 정보, 양측 서명·날인, ' +
      '계약일자, 특약사항, 분쟁해결 방법.',
    legalBasis: '결혼중개업법 제10조 ①, 시행규칙 제7조 별지 서식',
    penaltyRange:
      '1차 영업정지 1개월 → 2차 3개월 → 3차 6개월 → 4차 등록취소. 반복·상습 시 형사 §25.',
    disputeRisk:
      '서면계약 부존재는 소비자 주장대로 사실인정(민사소송법 §288 입증책임). ' +
  ...
  },
  {
    ...
    description:
      '계약서에 다음 금지행위 요소가 없는지 확인: (1) 미성년자·피성년후견인 중개, ' +
      '(2) 본인 동의 없는 신상정보 제3자 제공, (3) 강박·기망에 의한 혼인 유도, ' +
      '(4) 선불 고액 회비만 받고 중개 실적 없이 계약 종료, (5) 해외 현지 체류 강요.',
    legalBasis: '결혼중개업법 제11조 (결혼중개업자 등의 금지행위)',
    penaltyRange:
      '§11 위반: 3년 이하 징역 또는 3천만원 이하 벌금 (§25 1호). 등록취소 병과.',
    disputeRisk:
      '§11 위반 사실 적발 시 등록취소 + 형사처벌 + 손해배상 + 수수료 반환 4중 책임. ' +
      '업체 존립 가장 위협적 조항.',
  ...
  },
  {
    ...
    title: '단계별 환불조건 명시',
    description:
      '소비자분쟁해결기준(공정위 고시 제2024-X호) 국제결혼중개업: ' +
    ...
    penaltyRange:
      '불공정약관: 공정위 시정명령 + 약관법 §17 시정권고. ' +
      '소비자분쟁조정위원회 조정 신청 대응 필수.',
    disputeRisk:
      '환불조건이 소비자분쟁해결기준보다 불리한 경우 약관법 §6(신의성실원칙)으로 무효. ' +
  ...
  },
  {
    ...
    title: '손해배상 책임 조항 명시',
    description:
      '업체의 고의·중과실(허위 신상정보 제공, 사후관리 불이행 등)로 인한 손해 발생 시 ' +
      '배상 범위·산정기준 명시. 정액 상한을 두더라도 민법 §103 한계 내.',
    legalBasis: '결혼중개업법 제10조 ① 7호, 제24조의2, 민법 §750',
    penaltyRange:
      '§24의2 위반: 영업정지. 실제 분쟁은 민사소송으로 진행.',
    disputeRisk:
      '손해배상 상한을 "계약금의 2배"로 두는 관행이 많으나, 판례(대법 2018다248401 등)는 ' +
  ...
  },
  {
    ...
    description:
      '"성혼" 정의를 명확히 (혼인신고 완료? 맞선 성사? 입국? F-6 취득?). ' +
      '계약기간은 통상 2년, 성혼 미완성 시 연장·환불 조건 명시.',
    legalBasis: '결혼중개업법 제10조 ① 2호, 3호',
    penaltyRange: '시정명령',
    disputeRisk:
      '"성혼" 정의 모호는 분쟁 1위 원인. "맞선 3회 = 성혼"으로 주장하는 업체 vs ' +
  ...
  },
  {
    ...
    description:
      '자주 적발되는 불공정 조항: (1) 업체 면책 포괄조항, (2) 일방적 계약해지권, ' +
    ...
    disputeRisk:
      '약관법 §6 무효 판단 시 해당 조항만 무효(§16 일부무효) → 업체에 불리한 표준해석 적용.',
    dueDateRule: { type: 'relative', anchor: 'contract_signed', offsetDays: 0 },
  ...
  },
  {
    ...
    description:
      '직업·연소득(최근 3년)·재산·채무 서면 제공. F-6 비자 소득요건 연계. ' +
      '가구원수별 기준: 2인 기준 약 2,000만원, 3인 약 2,600만원(연도별 갱신).',
    legalBasis: '결혼중개업법 제10조의2 ① 4호, 출입국관리법 §10조의2',
    penaltyRange: '1차 영업정지 1개월 → 3개월 → 등록취소.',
    disputeRisk:
      '소득 허위 과장은 F-6 불허 원인이자 외국인 배우자 입장에서 "경제 기망" 주장 근거. ' +
      '혼인무효 인용 확률 상승.',
    dueDateRule: { type: 'relative', anchor: 'meeting_scheduled', offsetDays: -7 },
  ...
  },
  {
    ...
    title: '외국인 측 혼인경력 확인',
    description:
      '본국 혼인관계증명(베트남: 결혼상황확인서, 캄보디아: 독신증명서, 중국: 무혼인기록증명). ' +
      '아포스티유 또는 영사확인 필수. 자녀 유무·양육권 포함.',
    legalBasis: '결혼중개업법 제10조의2 ① 1호, ②, 국제사법 §63',
    penaltyRange: '1차 영업정지 1개월 → 3개월 → 등록취소.',
    disputeRisk:
      '본국 이혼 미정리 상태의 재혼은 중혼으로 혼인 무효(§815 1호). ' +
  ...
  },
  {
    ...
    title: '외국인 측 건강상태 확인',
    description:
      '본국 공인 의료기관 발급 건강진단서. HIV·결핵·B형간염·임신여부 포함. ' +
      'F-6 비자 심사 시 별도 제출 서류와 동일 기관 기준 권장.',
    legalBasis: '결혼중개업법 제10조의2 ① 2호, ②',
    penaltyRange: '1차 영업정지 1개월 → 3개월 → 등록취소.',
    disputeRisk:
      '임신 사실 미고지 후 입국 사례에서 혼인취소 청구 패턴. ' +
      '업체는 확인 의무 해태로 공동책임.',
    dueDateRule: { type: 'relative', anchor: 'meeting_scheduled', offsetDays: -7 },
  ...
  },
  {
    ...
    description:
      '양측이 상대방 신상정보를 서면으로 수령, 내용 설명을 듣고, 이해 확인 서명. ' +
      '통역 입회 + 타임스탬프 기록 필수. 제공 주체·일시·장소·방법 기록.',
    legalBasis: '결혼중개업법 제10조의2 ①, ④, 시행규칙 §8의2',
    penaltyRange: '1차 영업정지 1개월 → 3개월 → 등록취소.',
    disputeRisk:
      '"받았지만 설명 못 들었다" 주장 빈번. 설명 영상 녹화 또는 3자 입회 기록이 핵심.',
  ...
  },
  {
    ...
    description:
      '결혼중개업법 §10의2 이행 완료를 입증하는 증명서 발급. 3년간 의무 보관. ' +
      '행정처분·소송 시 결정적 증거.',
    legalBasis: '결혼중개업법 제10조의2 ④, 시행규칙 제8조 (보관의무 3년)',
    penaltyRange: '미보관: 과태료 500만원 이하. 허위보관: 형사 §25.',
    disputeRisk:
      '업체가 가장 많이 분실하는 서류. 행정처분 방어·민사 항변의 핵심 증거이므로 ' +
  ...
  },
  {
    ...
    description:
      '업체 대표·직원·가족 등 이해관계인은 통역 금지. ' +
      '중립 자격 통역자(한국어능력시험 + 해당국 언어 공인 자격 또는 공인번역사) 배치.',
    legalBasis: '결혼중개업법 제10조의2 ③, 시행규칙 §8의2',
    penaltyRange: '시정명령 → 영업정지 1개월.',
    disputeRisk:
      '이해관계인 통역은 "공정성 결여" 사유로 혼인무효 쟁점화. ' +
  ...
  },
  {
    ...
    disputeRisk:
      '본국법 최저연령 위반은 한국에서도 혼인 무효. F-6 비자 불허 직결.',
    dueDateRule: { type: 'relative', anchor: 'meeting_scheduled', offsetDays: 7 },
  ...
  },
  {
    ...
    title: '혼인신고 완료 증빙 확보',
    description:
      '혼인신고 수리증명서 또는 신혼 혼인관계증명서 확보. ' +
      'F-6 비자 신청의 필수 선행조건.',
    legalBasis: '가족관계의 등록 등에 관한 법률, 출입국관리법 §10의2',
    penaltyRange: '—',
    disputeRisk:
      '혼인신고 완료 없이 F-6 신청 시 자동 반려. 업체 중개 실패로 인정.',
  ...
  },
  {
    ...
    disputeRisk:
      '2020므15896 판결 이후 "혼인의사 부재" 혼인무효 인용 확대. ' +
  ...
  },
  {
    ...
    description:
      '필수 증빙: (1) 교제 사진 20장 이상(양측·시간대별), (2) 메시지·영상통화 기록, ' +
    ...
    disputeRisk:
      '의사소통 언어 부재가 2순위 불허 사유. 사전에 한국어·모국어 기초교육 필요.',
  ...
  },
  {
    ...
    description:
      '한국인 초청인 의무 이수. 온라인(IOM·법무부)·오프라인 가능. ' +
  ...
  },
  {
    ...
    description:
      '불허통지 수령 후 90일 이내 이의신청(§92의2) 또는 행정소송. ' +
      '이의신청 인용률 20-30%. 행정소송은 소득요건 미달 사안에서 실익 낮음.',
    legalBasis: '출입국관리법 제92조의2, 행정소송법',
    penaltyRange: '—',
    disputeRisk:
      '불허 사유가 "진정성 부족"인 경우 추가 증거 확보 후 6개월 후 재신청 권장. ' +
  ...
  // ===== Stage 6: 사후관리 (6) =====
  {
    ...
    title: '입국 후 1개월 사후관리',
    description:
      '안부 확인, 외국인등록(90일 이내 의무) 진행상황 점검, ' +
      '언어·문화 충격·시댁 갈등 초기 신호 감지.',
    legalBasis: '결혼중개업법 제10조의4, 다문화가족지원법 §6',
    penaltyRange: '시정명령 → 영업정지 15일.',
    disputeRisk:
      '1개월 내 "도주"가 사기결혼 의심 시점. 이 시점 기록이 업체 방어의 핵심.',
  ...
  },
  {
    ...
    title: '입국 후 6개월 사후관리',
    description:
      '임신·출산 여부, 경제생활, 취업 희망 등 생활 안정도 점검.',
    legalBasis: '결혼중개업법 제10조의4',
    penaltyRange: '시정명령.',
    disputeRisk:
      '임신 중 가출은 양육권·국적·체류 복합 분쟁. 사전 관리로 예방.',
  ...
  },
  {
    ...
    title: '다문화가족지원센터 연계',
    description:
      '관할 지역 다문화가족지원센터 프로그램(한국어·취업·육아·법률상담) 안내. ' +
      '1366 여성긴급전화, 다누리콜센터(1577-1366) 모국어 연결.',
    legalBasis: '다문화가족지원법 제6조, 제12조, 제12조의2',
    penaltyRange: '—',
    disputeRisk:
      '위기 상황에서 센터 연계 기록이 업체의 적극적 보호 의무 이행 증빙.',
    dueDateRule: { type: 'relative', anchor: 'entry_date', offsetDays: 30 },
