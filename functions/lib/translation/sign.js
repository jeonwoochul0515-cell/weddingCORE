...
export const signBackgroundTranslation = onCall({ region: REGION }, async (request) => {
    ...
    const { agencyId, clientId, infoId, lang, signerName } = request.data;
    ...
    const translationRef = db.doc(`agencies/${agencyId}/clients/${clientId}/backgroundInfo/${infoId}/translations/${lang}`);
    const [infoSnap, translationSnap] = await Promise.all([infoRef.get(), translationRef.get()]);
    if (!infoSnap.exists)
        throw new HttpsError('not-found', '신상정보 없음');
    const info = infoSnap.data();
    const isSource = info.sourceLang === lang;
    let translation = translationSnap.exists ? translationSnap.data() : null;
    // 원본 서명: 번역 문서가 없으면 원본 자체로 생성 후 서명
    if (isSource && !translation) {
        translation = {
            lang,
            fields: Object.fromEntries(Object.entries(info.fields).map(([k, v]) => [k, String(v)])),
            translatedBy: 'human',
    ...
    for (const key of REQUIRED_FIELDS) {
        ...
        if (srcVal === undefined || srcVal === null || String(srcVal).trim() === '')
            continue;
    ...
    if (missing.length > 0) {
        throw new HttpsError('failed-precondition', `번역 누락 필드: ${missing.join(', ')}`);
    }
    ...
    if (!isSource) {
        ...
        if (srcUpdatedMs && translatedMs && srcUpdatedMs > translatedMs) {
            throw new HttpsError('failed-precondition', '원본이 번역 이후 수정되었습니다. 번역을 재생성하세요.');
        }
    }
