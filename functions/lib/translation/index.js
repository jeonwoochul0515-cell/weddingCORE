...
export const translateBackgroundInfo = onCall({ region: REGION, timeoutSeconds: 120, memory: '512MiB', secrets: ['ANTHROPIC_API_KEY'] }, async (request) => {
    ...
    const { agencyId, clientId, infoId, targetLang } = request.data;
    ...
    if (!apiKey)
        throw new HttpsError('failed-precondition', 'ANTHROPIC_API_KEY 미설정');
    ...
    const fieldList = Object.entries(fields)
        ...
        .join('\n');
    const systemPrompt = `You are a certified legal translator specializing in Korean international marriage law ` +
    ...
    const userPrompt = `Translate the following fields to ${targetLangName}.\n\n` +
        `Fields:\n${fieldList}\n\n` +
